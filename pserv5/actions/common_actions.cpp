	...
	void Execute(DataActionDispatchContext& ctx) const override {
		...
		if (m_bCopyToClipboard) {
			...
			spdlog::info("Copied {} object(s) as {} to clipboard",
				ctx.m_selectedObjects.size(), exporter->GetFormatName());
		} else {
			// Export to file
			std::wstring defaultFileName = utils::Utf8ToWide(
				std::format("export_{}", ctx.m_pController->GetControllerName()));
			...
			if (utils::SaveFileDialog(ctx.m_hWnd,
									   L"Export Data",
									   defaultFileName,
									   filters,
									   0,
				...
				try {
					...
					MessageBoxA(ctx.m_hWnd,
						std::format("Failed to write file: {}", e.what()).c_str(),
						"Error", MB_OK | MB_ICONERROR);
				}
			}
		}
...
public:
	CopyAsJsonAction() : ExportAction{"Copy as JSON", "JSON", true} {}
};

class ExportToJsonAction final : public ExportAction {
public:
	ExportToJsonAction() : ExportAction{"Export to JSON...", "JSON", false} {}
};

class CopyAsTextAction final : public ExportAction {
public:
	CopyAsTextAction() : ExportAction{"Copy as Plain Text", "Plain Text", true} {}
};
