...
public:
	UninstallProgramAction() : DataAction{"Uninstall", ActionVisibility::ContextMenu} {}
	...
	void Execute(DataActionDispatchContext& ctx) const override {
		...
		if (MessageBoxA(ctx.m_hWnd, confirmMsg.c_str(), "Confirm Uninstallation", MB_YESNO | MB_ICONWARNING) == IDYES) {
			...
			if (reinterpret_cast<intptr_t>(result) <= 32) {
				...
				MessageBoxA(ctx.m_hWnd, std::format("Failed to launch uninstaller. Error: {}.", errorMsg).c_str(), "Uninstallation Error", MB_OK | MB_ICONERROR);
			} else {
