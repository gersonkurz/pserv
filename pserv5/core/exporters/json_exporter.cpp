...
namespace pserv
{
    ...
    std::string JsonExporter::ExportSingle(const DataObject *object, const std::vector<DataObjectColumn> &columns) const
    {
        if (!object)
            return "{}";
        ...
        for (size_t i = 0; i < columns.size(); ++i)
        {
            ...
            rapidjson::Value key{propertyName.c_str(), static_cast<rapidjson::SizeType>(propertyName.size()), allocator};
            rapidjson::Value value{propertyValue.c_str(), static_cast<rapidjson::SizeType>(propertyValue.size()), allocator};
    ...
    std::string JsonExporter::ExportMultiple(const std::vector<DataObject *> &objects, const std::vector<DataObjectColumn> &columns) const
    {
        if (objects.empty())
            return "[]";
