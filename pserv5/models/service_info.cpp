...
namespace pserv
{
    ...
    bool ServiceInfo::MatchesFilter(const std::string &filter) const
    {
        ...
        if (utils::ToLower(m_name).find(filter) != std::string::npos)
            return true;
        if (utils::ToLower(m_displayName).find(filter) != std::string::npos)
            return true;
        if (utils::ToLower(m_description).find(filter) != std::string::npos)
            return true;
        if (utils::ToLower(m_binaryPathName).find(filter) != std::string::npos)
            return true;
    ...
    std::string ServiceInfo::GetStatusString() const
    {
        ...
        case SERVICE_PAUSED:
            return "Paused";
        default:
            return std::format("Unknown ({})", m_currentState);
        }
    }
    ...
    std::string ServiceInfo::GetStartTypeString() const
    {
        ...
        case SERVICE_SYSTEM_START:
            return "System";
        default:
            return std::format("Unknown ({})", m_startType);
        }
    }
    ...
    std::string ServiceInfo::GetErrorControlString() const
    {
        ...
        case SERVICE_ERROR_CRITICAL:
            return "Critical";
        default:
            return std::format("Unknown ({})", m_errorControl);
        }
    }
